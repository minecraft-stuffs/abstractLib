...
import com.rhseung.abstractlib.api.file.path.URI.Companion.root
import com.rhseung.abstractlib.api.file.Nbt.NbtValue
import com.rhseung.abstractlib.api.utility.ErrorSolver.nsee
...
object Nbt {
		...
		override fun get(key: URI): NbtElement {
			...
			key.into { path ->
				...
				current = (current as NbtCompound).elements.find { it.key.toString() == path }
					?: nsee("NbtElement $path not found")
			}
	...
	fun builder(block: NbtCompoundBuilder.() -> Unit): NbtCompound {
		return NbtCompoundBuilder(root).apply(block).build()
	}
